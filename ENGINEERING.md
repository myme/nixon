...
| Keys in the picker | Actions: `Enter` run · `Alt-Enter` edit-before-run · `F1` show source · `F2` open in `$EDITOR` · `Tab` mark and move down (multi) · `Shift-Tab` mark and move up · `Esc`/`Ctrl-C` cancel. Marks are keyed by candidate identity, not row position, so they survive a query change: confirming returns every marked row in list order, including rows the current query no longer matches, and the `(n)` counter is the total marked rather than the visible ones. List: `Ctrl-N`/`Ctrl-J`/`Down` next, `Ctrl-P`/`Ctrl-K`/`Up` previous, `PgDn`/`Ctrl-V` and `PgUp`/`Alt-V` a full page, `Alt-J`/`Alt-K` half a page (page = visible rows, half = rows/2, both at least one). `Ctrl-D`/`Ctrl-U` are readline's in the query line, not fzf's list moves. Query line follows **readline/fzf defaults**: `Ctrl-A`/`Home`, `Ctrl-E`/`End`, `Ctrl-B`/`Left`, `Ctrl-F`/`Right`, `Alt-B`/`Alt-F` word, `Ctrl-H`/`Backspace`, `Ctrl-D`/`Delete`, `Ctrl-W`/`Alt-Backspace` delete word back, `Alt-D` delete word forward, `Ctrl-U` delete to start, `Ctrl-Y` yank. The terminal cursor is shown at the query position; matched characters are highlighted in the list (nucleo match indices), like fzf. |
| `-1` auto-select | Kept: a query (from the CLI positional) that matches exactly one candidate selects it without opening the picker. With **streaming** candidates the terminal is taken lazily: nothing is drawn while `-1` could still apply, and the picker opens the moment a second candidate matches. A command whose whole output is one line therefore blocks until it exits (as in v1). `-1` is decided **once**, on the opening query — never re-checked while the user types. |
| Streaming placeholders | `Lines`/`Fields` candidates stream from the command into the picker as they arrive; `Columns`/`JSON` are buffered (widths / whole document needed). The streamed child leads its own process group and cancel signals the **group** (`killpg`), since the work happens in the interpreter's children, not the interpreter. |
